    ...
    match operation:
        case 'run':
            fake_CMD_options={
                ...
                'interactive':False,'config_file':None,'debug':False,
                'version':'1.0.4'
                }
        case 'build':
            fake_CMD_options={
                'operation':f'{operation}','model_name':'test',
                'writable':False,'force':False,
        ...
        case 'list':
            fake_CMD_options={
                'operation':'list','group':'','model_name':'',
                'long_desc':False,'config_file':None,'debug':False,
                'version':'1.0.4'
                }
        case 'start':
            fake_CMD_options={
...
def test_config_not_yaml():
    with pytest.raises(ValueError):
        load_container_config_file(f"{DATA_DIR}/test1.txt")

def test_no_description():
    with pytest.raises(exceptions.MissingValueError):
        load_container_config_file(f"{DATA_DIR}/test1.yaml")

def test_no_image():
    toolkit_home = get_toolkit_home()
    Containers = load_container_config_file(f"{DATA_DIR}/test2.yaml")
...
def test_invalid_image():
    ...
    with pytest.raises(ValueError):
       load_container_config_file(f"{DATA_DIR}/test9.yaml")


def test_invaild_model_name():
    with pytest.raises(ValueError):
        load_container_config_file(f"{DATA_DIR}/test7.yaml")

def test_no_shared_dir():
    with pytest.raises(FileNotFoundError):
        load_container_config_file(f"{DATA_DIR}/test4.yaml")

def test_shared_dir_is_file():
    with pytest.raises(ValueError):
        load_container_config_file(f"{DATA_DIR}/test4b.yaml")

def test_multi_definition_1():
    load_container_config_file(f"{DATA_DIR}/test5.yaml")

def test_invalid_option():
    # check that we raise error if we provide an invalid option
    with pytest.raises(ValueError):
        load_container_config_file(f"{DATA_DIR}/test10a.yaml")

def test_case_insenstive():
    load_container_config_file(f"{DATA_DIR}/test10b.yaml")

def test_multi_definition_2():
    # chekc that we raise error if model names are repeated in the same config file
    with pytest.raises(DuplicateKeyError):
        load_container_config_file(f"{DATA_DIR}/test6.yaml")

def test_multi_definition_3():
# check for that we raise an error if model names are repeated 
# across multiple config files
    with pytest.raises(DuplicateKeyError):
        containers = check_container_config([f"{DATA_DIR}/valid.yaml",f"{DATA_DIR}/valid.yaml"])
...
def test_format_command(build_test_container):
    ...
    Containers = load_container_config_file(f"{DATA_DIR}/valid.yaml")
...
def test_run_with_no_image():
    ...
    with pytest.raises(FileNotFoundError):
        Containers = load_container_config_file(f"{DATA_DIR}/test8.yaml")
